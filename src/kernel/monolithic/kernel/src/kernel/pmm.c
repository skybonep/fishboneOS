...
void pmm_init(multiboot_info_t *mbinfo)
{
    ...
    // 4. THE HARD TRUTH: Protect the Kernel image.
    ...
    // 5. Protect the first 1 MiB for BIOS, bootloader, and interrupt tables.
    uint32_t bios_frames = (BIOS_BOOT_RESERVED_END / PAGE_SIZE) + 1;
    for (uint32_t i = 0; i < bios_frames; i++)
    {
        pmm_set_bit(i);
    }
